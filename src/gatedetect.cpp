...
int ddepth=CV_16S,scale=1,delta=0,h=3,s=0,v=0,hmax=14,smax=255,vmax=255,erosion_size=0,morph_size=1,size=10,line_parameter=15,min_line_length=102,max_line_gap=30,reflection_deleting_parameter=30;
vector<Vec4i> lines;
int state=0;
Point cent;
int main(int argc,char** argv)
{
  ...
  for( size_t i = 0; i < lines.size(); i++ )
  {
    ...
    //cout<<angle<<endl;
        if(abs(90.0-angle) < line_parameter)//&&((l[1]>480-reflection_deleting_parameter)||(l[3]>480-reflection_deleting_parameter)))
				{
					/*for(size_t j=0;j<lines.size();j++)
					{
						Vec4i l2=lines[j];
						if((abs(l[0]-l2[0])>200) && abs(abs(l[1]-l[3])-abs(l2[1]-l2[3]))<20)
						{
							cout<<"\tSTATE-1";
							line(src,Point(l[0],l[1]),Point(l[2],l[3]),Scalar(0,0,255),8,CV_AA);
							line(src,Point(l2[0],l2[1]),Point(l2[2],l2[3]),Scalar(0,0,255),8,CV_AA);
						...
						if(a==-1)
						{
							...
							if(b==-1 && abs(lines[a][0]-l[0])>200 && abs(abs(lines[a][1]-lines[a][3])-abs(l[1]-l[3]))<50)
							{
								state=1;
								cout<<"\tSTATE-1 ";
								b=i;
								line( src, Point(lines[b][0], lines[b][1]), Point(lines[b][2], lines[b][3]), Scalar(0,0,255), 8, CV_AA);
							}
						else if(b==-1 &&  abs(lines[a][0]-l[0])>100 && abs(abs(lines[a][1]-lines[a][3])-abs(l[1]-l[3]))>state_2_parameter)
							{
								state=2;
								cout<<"\tSTATE-2 ";
								b=i;
							...
							if(state==1)
							{
								cent.x=(lines[a][0]+lines[b][0])/2;
								cent.y=(lines[a][1]+lines[a][3])/2;
							}
						else	if(state==2)
							{
								cent.x=(lines[a][0]+lines[b][0])/2;
								cent.y=max(lines[b][1],lines[b][3]);
							}
